...
ROUTE_POLICIES = {
    ROUTE_PROFESSIONAL_OVERVIEW: "Answer as professional positioning. Give the visitor the essential frame: cross-domain systems experience, reliability/governance/platform thinking, and AI/data direction. Do not list every employer unless needed.",
    ROUTE_HIRING_FIT: "Answer hiring, recruiting, placement, selection, candidate, offer, competition, or role-fit questions directly. Say Rajesh is 100% hire-worthy and frame it as a no-brainer for roles needing systems thinking, AI/platform judgment, analytics, governance, reliability, and cross-domain execution. Reason the decision from his skills, education credentials, and experience. Position him as a differentiated professional brand, not a generic candidate. Do not give a generic biography.",
    ROUTE_WORK_EXPERIENCE: "Answer as a work-experience deep dive. If this is a second turn on the same experience, go deeper into mechanisms, constraints, stakeholders, metrics, systems built, and execution details. Do not repeat the first overview.",
    ROUTE_AI_TECHNICAL_DEPTH: "Answer as AI/data/technical depth. Connect mathematics, probability, statistics, analytics, modelling, ML, deep learning, MLOps, GenAI, platform engineering, and domain systems only where supported.",
    ROUTE_COMPARISON: "Answer as comparison and synthesis. Compare the experiences across systems, stakeholders, governance, risk, decision quality, operating constraints, and AI/platform relevance. Do not merely list facts from each side.",
    ROUTE_COLLABORATION_FIT: "Answer as collaboration fit. Explain project contexts, partner scenarios, and operating environments where Rajesh's systems, AI/platform, analytics, and execution background would be useful.",
    ROUTE_INTERVIEW_EVALUATION: "Answer as practical evaluation guidance. Give interview signals for systems thinking, AI/data reasoning, ambiguity handling, execution depth, governance judgment, and leadership potential.",
    ROUTE_GUIDED_DISCOVERY: "Answer as guided discovery for a passive visitor. Create curiosity with one concrete professional thread and one intellectual AI/data/systems thread. Do not overwhelm the visitor.",
}

ROUTE_FOLLOWUPS = {
    ROUTE_PROFESSIONAL_OVERVIEW: ["Explore his work experience.", "See AI/platform direction."],
    ROUTE_HIRING_FIT: ["Assess AI platform fit.", "What interview signal matters?"],
    ROUTE_AI_TECHNICAL_DEPTH: ["Connect this to work experience.", "Go deeper into modelling."],
    ROUTE_COLLABORATION_FIT: ["Explore collaboration scenarios.", "Assess role fit."],
    ROUTE_INTERVIEW_EVALUATION: ["Test AI/platform judgment.", "Test systems thinking."],
    ROUTE_GUIDED_DISCOVERY: ["Start with real-world systems.", "Try AI reasoning thread."],
    ROUTE_OUT_OF_SCOPE: ["Ask about his work.", "Ask about collaboration fit."],
}
...
def lambda_handler(event: Mapping[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        route = classify_route(question, conversation_context)
        ...
        answer = _enforce_bulleted_answer(
            invoke_nova_pro(question=question, conversation_context=conversation_context, route=route),
            question=question,
...
def invoke_nova_pro(question: str, conversation_context: Iterable[Mapping[str, str]] | None = None, route: str = ROUTE_PROFESSIONAL_OVERVIEW) -> str:
    ...
    user_prompt = _build_user_prompt(question, EMBEDDED_KNOWLEDGE_CONTEXT, EMBEDDED_SOURCES, conversation_context or [], route)
...
def _build_user_prompt(
    ...
    route_policy = ROUTE_POLICIES.get(route, ROUTE_POLICIES[ROUTE_PROFESSIONAL_OVERVIEW])
    return f"""
Visitor question:
{question}

Detected conversation route:
{route}

Route-specific answer policy:
{route_policy}
...
def classify_route(question: str, conversation_context: Iterable[Mapping[str, str]]) -> str:
    text = question.lower().strip()
    if _is_private_or_unsupported(question):
        return ROUTE_OUT_OF_SCOPE
    if _is_greeting(question):
        return ROUTE_GREETING
    if _matches(text, r"\b(should i hire|hire him|hire worthy|hire[- ]?worthy|hiring|recruit|recruiter|recruiting|recruitment|placement|place him|placed|job fit|role fit|fit for (this )?role|suitable for|good candidate|candidate|shortlist|short-listed|select him|selection|offer him|make an offer|employ|employment|employee|onboard|onboarding|staffing|talent|talent fit|appoint|appointment|engage him|retain him|worth hiring|is he worth|no-brainer|competition|competitive|compete|brand of his own)\b"):
        return ROUTE_HIRING_FIT
    if _matches(text, r"\b(interview|evaluate|evaluation|what should i test|how should i test|screen him|assessment|signal)\b"):
        return ROUTE_INTERVIEW_EVALUATION
    if _matches(text, r"\b(collaborate|collaboration|partner|partnership|work with him|project with him|consulting|client)\b"):
        return ROUTE_COLLABORATION_FIT
    if _matches(text, r"\b(compare|versus| vs |difference between|how do .* connect|synthesis|relate)\b"):
        return ROUTE_COMPARISON
    if _matches(text, r"\b(ai|genai|mlops|machine learning|deep learning|data model|data modelling|data modeling|analytics|probability|statistics|statistical|math|mathematics|uncertainty|model|modelling|modeling)\b"):
        return ROUTE_AI_TECHNICAL_DEPTH
    if _matches(text, r"\b(bpcl|bharat petroleum|medtronic|supreme court|smaat|r-cafe|rcafe|redrybbons|red rybbons|work experience|real-world work|experience|project|proof point|reliability|asset governance|ecosystem design|control planes)\b"):
    ...
    if _matches(text, r"\b(who is|tell me about|what should i know|about him|about rajesh|professional background|profile|overview)\b"):
        return ROUTE_PROFESSIONAL_OVERVIEW
    if _is_contextual_followup(question, conversation_context):
        return _route_from_context(conversation_context)
    return ROUTE_PROFESSIONAL_OVERVIEW
...
def _route_from_context(conversation_context: Iterable[Mapping[str, str]]) -> str:
    recent = " ".join(str(item.get("text") or "") for item in conversation_context[-4:] if isinstance(item, Mapping)).lower()
    if "hire" in recent or "role fit" in recent:
        return ROUTE_HIRING_FIT
    if "compare" in recent:
        return ROUTE_COMPARISON
    if _latest_experience_topic(recent):
        return ROUTE_WORK_EXPERIENCE
    if _matches(recent, r"\b(ai|mlops|genai|probability|statistics|analytics|modelling|modeling)\b"):
        return ROUTE_AI_TECHNICAL_DEPTH
    return ROUTE_GUIDED_DISCOVERY
...
def _enforce_bulleted_answer(answer: str, question: str = "", conversation_context: Iterable[Mapping[str, str]] | None = None, route: str = ROUTE_PROFESSIONAL_OVERVIEW) -> str:
    ...
    if _has_answer_bullets(lines):
        return _normalize_bulleted_response(lines, context_text, route)
    ...
    return "\n".join(bullets + _finalize_followups(followups, body_text + " " + context_text, route))


def _normalize_bulleted_response(lines: list[str], context_text: str = "", route: str = ROUTE_PROFESSIONAL_OVERVIEW) -> str:
    list_items = []
    other_lines = []
    for line in lines:
    ...
    answer_lines = _route_answer_lines(answer_lines[:3], route)
    return "\n".join(answer_lines + _finalize_followups(followups, topic_text, route))



def _route_answer_lines(answer_lines: list[str], route: str) -> list[str]:
...
def _finalize_followups(followups: list[str], topic_text: str, route: str = ROUTE_PROFESSIONAL_OVERVIEW) -> list[str]:
    cleaned = [_shorten_sentence(_clean_followup_label(item), 7) for item in followups if item]
    planned = _route_followups(route, topic_text)
...
def _route_followups(route: str, topic_text: str) -> list[str]:
    if route in {ROUTE_WORK_EXPERIENCE, ROUTE_COMPARISON}:
        return _planned_followups(topic_text)
    return ROUTE_FOLLOWUPS.get(route, [])
