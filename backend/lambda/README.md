...
The Lambda now supports a structured conversation controller. The model no longer owns follow-up options.
...
- `answer` contains only the 3 answer bullets.
- `options` is a separate array generated by Lambda code.
- `conversation_state` is returned every request and should be sent back on the next turn.
- `conversation_context` remains accepted for backward compatibility, but new clients should use `chat_history`.

New test fixtures:

```text
backend/lambda/tests/postman_stateful_body.json
backend/lambda/tests/test_lambda_stateful_event.json
backend/lambda/tests/postman_hiring_body.json
backend/lambda/tests/test_lambda_hiring_event.json
```
Seeded journey behavior:

- A new frontend session creates one `journey_seed`.
- `experience_sequence` rotates from that seed.
- The session remains deterministic after the seed is chosen.
- This avoids every visitor starting with BPCL while preserving controlled flow.
